<script lang="ts">
  ...
  async function copyPath(){
    ...
    try{await navigator.clipboard.writeText(detail.assetPath);onNotify(messages.toast.copiedPath,'success')}catch{onNotify(messages.workspace.notices.copyFailed,'error')}
  }
</script>
...
<article class="quick-detail">
  ...
  {:else if detail.kind==='image'||detail.kind==='file'}<div class="file-info"><strong>{detail.fileName}</strong>{#if !detail.available}<span>{detail.kind==='image'?messages.workspace.unavailableImage:messages.workspace.unavailableFile}</span>{/if}{#if detail.summary.capabilities.copyPath}<button type="button" class="copy-path" onclick={copyPath} disabled={!detail.available}>{messages.entry.copyPath}</button>{/if}</div>
  {:else}
