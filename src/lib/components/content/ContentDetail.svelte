<script lang="ts">
  ...
  interface Props { detail:Detail; resetToken:string|number; onClose:()=>void; onChanged:(id:string)=>Promise<void>; onNotify:(message:string,kind?:'success'|'error')=>void; onDelete?:()=>void; onToggleSaved?:()=>void }
  let { detail, resetToken, onClose, onChanged, onNotify, onDelete, onToggleSaved }:Props=$props()
</script>
<section class="detail-shell" aria-label="内容详情">
...
<style>.detail-shell{height:100%;min-width:0;overflow:auto;background:var(--surface-0)}</style>
